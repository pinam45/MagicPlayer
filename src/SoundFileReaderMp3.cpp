...
namespace {
	...
	void set_mpg123_quiet(mpg123_handle* handle){
		...
		if(error != MPG123_OK){
			sf::err() << "Failed to get mpg123 parameters: " << mpg123_plain_strerror(error) << std::endl;
		}

		error = mpg123_param(handle, MPG123_FLAGS, mpg123_flags | MPG123_QUIET, dummy);
		if(error != MPG123_OK){
			sf::err() << "Failed to set mpg123 parameters: " << mpg123_plain_strerror(error) << std::endl;
		}
	}
}
...
bool SoundFileReaderMp3::check(sf::InputStream& stream) {
	...
	if(error != MPG123_OK){
		sf::err() << "Failed to open mpg123 for reading: " << mpg123_plain_strerror(error) << std::endl;
		mpg123_exit();
		return false;
	}
	set_mpg123_quiet(handle);

	error = mpg123_replace_reader_handle(handle, &mpg123_handle_read, &mpg123_handle_lseek, &mpg123_handle_cleanup);
	if(error != MPG123_OK){
		sf::err() << "Failed to replace mpg123 reader handle: " << mpg123_plain_strerror(error) << std::endl;
	...
	if(error != MPG123_OK){
		sf::err() << "Failed to open handle with mpg123: " << mpg123_plain_strerror(error) << std::endl;
		mpg123_delete(handle);
		mpg123_exit();
		return false;
	}
	...
	if(error != MPG123_OK && error != MPG123_NEW_FORMAT){
		sf::err() << "Failed to scan stream with mpg123: " << mpg123_plain_strerror(error) << std::endl;
		mpg123_close(handle);
		mpg123_delete(handle);
		mpg123_exit();
		return false;
...
bool SoundFileReaderMp3::open(sf::InputStream& stream, sf::SoundFileReader::Info& info) {
	...
	error = mpg123_replace_reader_handle(m_handle, &mpg123_handle_read, &mpg123_handle_lseek, &mpg123_handle_cleanup);
	if(error != MPG123_OK){
		sf::err() << "Failed to replace mpg123 reader handle: " << mpg123_plain_strerror(error) << std::endl;
		return false;
	}
	...
	if(length <= 0){
		sf::err() << "Failed to get stream length with mpg123: " << mpg123_plain_strerror(error) << std::endl;
		return false;
	}
	...
	if(error != MPG123_OK
	&& error != MPG123_NEW_FORMAT
	&& rate > 0
	&& channels > 0
	){
		sf::err() << "Failed to get stream format with mpg123: " << mpg123_plain_strerror(error) << std::endl;
		return false;
	}
...
void SoundFileReaderMp3::seek(sf::Uint64 sampleOffset) {
	const off_t new_offset = mpg123_seek(m_handle, static_cast<off_t>(sampleOffset) / m_channelCount, SEEK_SET);
	if(new_offset < 0){
		sf::err() << "Failed to seek with mpg123: " << mpg123_plain_strerror(static_cast<int>(new_offset)) << std::endl;
	}
}
...
sf::Uint64 SoundFileReaderMp3::read(sf::Int16* samples, sf::Uint64 maxCount) {
	size_t done = 0;
	const int error = mpg123_read(m_handle, reinterpret_cast<unsigned char*>(samples), maxCount * sizeof(sf::Int16), &done);
	if(error != MPG123_OK && error != MPG123_NEW_FORMAT){
		sf::err() << "Failed to read stream with mpg123: " << mpg123_plain_strerror(error) << std::endl;
	}
